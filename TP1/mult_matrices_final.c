...
/*     for (i = 0; i < N; i += tam_bloque){
    ...
    time = dwalltime() - tick;
    printf("Tiempo requerido para calcular la multiplicacion: %f\n",time);
    printf("matriz: %dx%d\n",N,N);
    printf("tam_bloque: %d\n",tam_bloque);
    printf("N = %d\n",N);
    printf("imprimo primer y ultimo elemento arreglo, deben ser N c/u : [%0.0f] [%0.0f] \n",C[0],C[N*N-1]);
    /*printf("imprimo C: cada elemento debe dar N \n");
     for(i=0;i<N;i++){
        for(j=0;j<N;j++){
